...
>
> **Dossier projet pour cette pratique :** `langchain-course-project-hello-world-local/`
...
| 7 | [Étape 5 — Configurer le `.env` (minimal)](#section-7) |
...
| 12 | [Choix du modèle Ollama (`gemma3:270m`, `qwen3:1.7b`, etc.)](#section-12) |
| 13 | [Pièges spécifiques à Ollama](#section-13) |
| 14 | [Combien ça consomme (RAM, disque, CPU/GPU)](#section-14) |
...
- Lancer le projet `langchain-course-project-hello-world-local/` en `venv + pip`, `uv`, ou Docker.
...
> [!NOTE]
> `gemma3:270m` est volontairement **petit** pour démarrer vite, même sur un ordinateur modeste. La qualité est limitée mais largement suffisante pour ce Hello World. Pour la suite du cours, vois la [section 12](#section-12) pour des modèles plus gros.
...
> [!IMPORTANT]
> Toutes les commandes de ce guide se lancent depuis le **dossier du projet** : `langchain-course-project-hello-world-local/`. C'est ce dossier qui contient `main.py`, `Dockerfile`, `pyproject.toml`, `.env.example`. Si tu as cloné le cours complet, c'est `repositories/Module00-intro-langchain-ia-agentique/langchain-course-project-hello-world-local/`. Si tu as juste reçu le dossier zippé, **tu es déjà dedans** dès que tu l'ouvres.
...
> [!NOTE]
> Le `main.py` du dossier `-local/` est **déjà configuré pour Ollama** (`ChatOllama`, `MODEL_NAME = "gemma3:270m"`, `OLLAMA_BASE_URL` lu depuis l'env). Tu n'as **pas** à modifier `main.py` à la main : c'est la différence majeure avec le dossier `-openai/`.
...
## 7. Étape 5 — Configurer le `.env` (minimal)

Avec Ollama, **aucune clé API n'est requise**. Mais le projet utilise `python-dotenv` qui lit un `.env`. Crée-en un à partir du gabarit :

```bash
cp .env.example .env
```

Le `.env.example` ne contient qu'une seule variable optionnelle, `OLLAMA_BASE_URL`, qui sert à indiquer où Ollama écoute :

| Mode d'exécution | Valeur recommandée | Action |
|---|---|---|
| `venv` ou `uv` (natif) | (rien, le code utilise `http://localhost:11434` par défaut) | Laisse `.env` vide |
| Docker | `OLLAMA_BASE_URL=http://host.docker.internal:11434` | Décommente la ligne dans `.env` |

> [!IMPORTANT]
> Pour la méthode Docker (§ [10](#section-10)), il **faut** décommenter `OLLAMA_BASE_URL=http://host.docker.internal:11434` sinon le conteneur essaiera `localhost:11434` qui désigne le conteneur lui-même, pas l'hôte où tourne Ollama.
...
| Commande | Rôle |
|---|---|
| `uv sync` | Crée `.venv/` et installe les deps (et génère `uv.lock` si absent) |
| `uv run python main.py` | Lance Python dans le venv (sans avoir à l'activer) |

### 9.3 Toutes les commandes `uv` utiles
...
> [!IMPORTANT]
> Ollama tourne sur ton **hôte** (Windows / macOS / Linux), pas dans le conteneur. Le conteneur doit donc atteindre l'hôte. Sur **Windows et macOS**, Docker Desktop fournit `host.docker.internal` automatiquement. Sur **Linux**, le `docker-compose.yml` du dossier `-local/` ajoute `extra_hosts: host.docker.internal:host-gateway` pour rendre le projet portable.

### 10.1 Vérifie que `.env` contient le bon `OLLAMA_BASE_URL`

Ouvre `.env` et assure-toi d'avoir cette ligne (décommentée) :

```
OLLAMA_BASE_URL=http://host.docker.internal:11434
```

Sans ça, le conteneur tentera `http://localhost:11434` (= lui-même) et obtiendra `Connection refused`.

### 10.2 Mode foreground (par défaut)
...
### 10.4 Différence `--build` vs sans `--build`
...
### 10.5 Toutes les commandes Docker utiles
...
### 10b.1 Pourquoi `docker exec` ne marche pas directement

Le réflexe Google c'est :

```bash
docker compose up -d                              # demarrer en arriere-plan
docker exec -it langchain-hello-world-local bash  # entrer dans le conteneur
```
...
env | grep OLLAMA                                  # OLLAMA_BASE_URL=http://host.docker.internal:11434
...
>>> llm = ChatOllama(model="qwen3:1.7b", base_url=base)
...
```
========================================================================
  LangChain Hello World - Ollama local - PromptTemplate + LCEL
========================================================================

[1/4] Construction du PromptTemplate (le 'formulaire' reutilisable)
      [OK] Variables du template : ['information']

[2/4] Initialisation du LLM Ollama 'gemma3:270m' (le 'cerveau')
      Endpoint : http://localhost:11434
      [OK] ChatOllama pret

[3/4] Composition de la chaine LCEL : prompt | llm  (le 'tuyau')
      [OK] Chaine construite. Reutilisable a l'infini.
...
------------------------------------------------------------------------
  1/3 - Elon Musk
------------------------------------------------------------------------
Appel LLM en cours (peut prendre 5-60s selon la machine)... [OK] 8.3s

# Resume
Elon Musk est un entrepreneur sud-africain ...
...
========================================================================
  Termine en 26.7s (3/3 reussis)
========================================================================
  Modele utilise       : gemma3:270m (Ollama, 100% local)
  Endpoint Ollama      : http://localhost:11434
  Cout                 : 0.00 USD (gratuit, tout tourne sur ta machine)
  La meme chaine LCEL a ete invoquee 3 fois sans rebuild.
```

> [!NOTE]
> Avec un petit modèle comme `gemma3:270m`, la qualité est notablement inférieure à `gpt-4o-mini`. Tu peux observer des incohérences, des fautes ou des réponses incomplètes. Pour une qualité supérieure, voir la [section 12](#section-12).

> [!NOTE]
> Si un appel échoue (Ollama pas démarré, modèle absent, etc.), tu verras `[ERREUR] <Type>: <message>` puis le script **continue** avec le personnage suivant.
...
## 12. Choix du modèle Ollama (`gemma3:270m`, `qwen3:1.7b`, etc.)
...
Puis dans `main.py`, change la variable `MODEL_NAME` (en haut du fichier) :
...
## 13. Pièges spécifiques à Ollama
...
| Docker : `Connection refused: host.docker.internal:11434` | Le conteneur n'atteint pas l'hôte | Vérifier `OLLAMA_BASE_URL` dans `.env`, `extra_hosts` dans `docker-compose.yml` (Linux), redémarrer Docker Desktop (Win/Mac) |
| Réponse extrêmement lente | Modèle trop gros pour ton CPU/RAM | Prendre un modèle plus petit (voir section 12) |
...
| `Endpoint : http://localhost:11434` dans Docker | Tu n'as pas mis `OLLAMA_BASE_URL=http://host.docker.internal:11434` dans `.env` | Décommenter la ligne dans `.env` puis `docker compose up --build` |
...
## 14. Combien ça consomme (RAM, disque, CPU/GPU)
...
# Aller dans le dossier du projet (langchain-course-project-hello-world-local/)
ls
# main.py, pyproject.toml, Dockerfile, docker-compose.yml, .env.example...

# Creer le .env (meme avec Ollama, python-dotenv le lit)
cp .env.example .env
# Pour Docker, edite .env et decommente :
#   OLLAMA_BASE_URL=http://host.docker.internal:11434
```
...
# Verifier que .env contient OLLAMA_BASE_URL=http://host.docker.internal:11434
...
### 15.6 Méthode D — Mode interactif Docker
...
env | grep OLLAMA                         # verifier OLLAMA_BASE_URL
python --version
...
### 15.7 Méthode D bis — `docker exec` (cas avancé)

```bash
# Demarrer un conteneur en sleep infinity
docker compose run -d --name hello-shell-local --entrypoint sleep app infinity

# Entrer
docker exec -it hello-shell-local bash
python main.py
exit                                      # le conteneur continue de tourner

# Stopper et supprimer manuellement
docker stop hello-shell-local
docker rm hello-shell-local
```

### 15.8 Gestion des modèles Ollama
