...
Ce script demontre les bases de LangChain :
  ...
  2. Initialisation d'un LLM OpenAI.
...
Pour la version Ollama 100% locale et gratuite, voir le dossier
langchain-course-project-hello-world-local/ (Pratique 2).
"""
...
def main() -> None:
    banner("LangChain Hello World - OpenAI - PromptTemplate + LCEL")
...
"""
    ...
    print(f"\n[2/4] Initialisation du LLM OpenAI '{MODEL_NAME}' (le 'cerveau')")
    ...
    if cout_total > 0:
        ...
        if succes:
            print(f"  Cout moyen par appel : ${cout_total / succes:.5f} USD")
    print(f"  Modele utilise       : {MODEL_NAME} (OpenAI)")
