...
- Lancer le projet `langchain-course-project-hello-world-openai/` en **3 méthodes distinctes** : `venv + pip` (classique), `uv` (moderne), Docker (isolé).
...
> [!IMPORTANT]
> Toutes les commandes de ce guide se lancent depuis le **dossier du projet** : `langchain-course-project-hello-world-openai/`. C'est ce dossier qui contient `main.py`, `Dockerfile`, `pyproject.toml`, `.env.example`. Si tu as cloné le cours complet, c'est `repositories/Module00-intro-langchain-ia-agentique/langchain-course-project-hello-world-openai/`. Si tu as juste reçu le dossier zippé, **tu es déjà dedans** dès que tu l'ouvres.
...
> [!NOTE]
> Le dossier ne contient pas de `uv.lock` versionné : il est régénéré automatiquement par `uv sync` (méthode B) ou par le `Dockerfile` (méthode C). Aucune action de ta part.
...
> [!NOTE]
> Le `.env` doit être dans le **dossier intérieur** (`langchain-course-project-hello-world-openai/`), **pas** à la racine du cours. C'est l'emplacement où `main.py` cherche les variables d'environnement.
...
> [!NOTE]
> Le `Dockerfile` télécharge Python 3.12-slim, installe `uv`, copie `pyproject.toml`, fait `uv sync` (qui génère le lock + installe les deps), puis lance `python main.py`. Le `.env` est passé via `env_file:` dans `docker-compose.yml`. Le conteneur s'appelle `langchain-hello-world-openai`.
...
docker exec -it langchain-hello-world-openai bash  # entrer dans le conteneur
```
...
| `bash` | La commande à exécuter dans le conteneur, à la place de `python main.py` |
 ...
 > [!IMPORTANT]
> `app` est le nom du service défini dans `docker-compose.yml` (`services: app:`). `langchain-hello-world-openai` c'est le `container_name` qui s'applique seulement avec `docker compose up`. Avec `docker compose run`, Docker crée un conteneur jetable nommé automatiquement.
...
```
========================================================================
  LangChain Hello World - OpenAI - PromptTemplate + LCEL
========================================================================

[1/4] Construction du PromptTemplate (le 'formulaire' reutilisable)
      [OK] Variables du template : ['information']

[2/4] Initialisation du LLM OpenAI 'gpt-4o-mini' (le 'cerveau')
      [OK] ChatOpenAI pret
...
------------------------------------------------------------------------
  1/3 - Elon Musk
------------------------------------------------------------------------
Appel LLM en cours (peut prendre 2-30s selon le modele)... [OK] 4.5s

# Resume
Elon Musk, entrepreneur sud-africain, est le fondateur de SpaceX et Tesla...

## Faits interessants peu connus
1. Avant de co-fonder PayPal, Musk a lance Zip2...
2. Musk a propose le concept de transport futuriste Hyperloop...

## Question ouverte a creuser
Comment les innovations d'Elon Musk affectent-elles la durabilite ?

[i] Tokens : 125 entree + 145 sortie = 270 total
[i] Cout estime : $0.00011 USD
...
========================================================================
  Cout total estime    : $0.0003 USD
  Cout moyen par appel : $0.00011 USD
  Modele utilise       : gpt-4o-mini (OpenAI)
...
> [!NOTE]
> Si un appel échoue (rate limit, timeout, etc.), tu verras `[ERREUR] <Type>: <message>` puis le script **continue** avec le personnage suivant. Le résumé final indique combien d'appels ont réussi.
...
Le code par défaut utilise `gpt-4o-mini` (variable `MODEL_NAME` en haut de `main.py`) :

```python
MODEL_NAME = "gpt-4o-mini"
```

C'est le **meilleur rapport qualité/prix** pour débuter et c'est largement suffisant pour ce projet. Si tu veux tester un modèle plus puissant, change la valeur :

```python
MODEL_NAME = "gpt-4o"   # ou "gpt-5"
```
...
> [!TIP]
> Le projet Hello World consomme environ **800 tokens par appel** sur les 3 personnages combinés (prompt + réponse). Avec `gpt-4o-mini`, ça fait ≈ **0,0003 USD par exécution complète**. Tu peux relancer le projet **30 000 fois** avec 10 USD.

> [!NOTE]
> Le dictionnaire `TARIFS_OPENAI` dans `main.py` ne connaît que `gpt-4o-mini`, `gpt-4o` et `gpt-5`. Si tu utilises un autre modèle, le coût ne sera juste pas estimé (le reste fonctionne).
...
| `openai.NotFoundError: model '...' does not exist` | Ton compte n'a pas accès à ce modèle | Remplacer `MODEL_NAME` par `gpt-4o-mini` dans `main.py` |
...
- 1 exécution complète (3 personnages) ≈ **0,0003 USD** (trois dix-millièmes de dollar)
- 100 exécutions ≈ **0,03 USD** (3 cents)
- 10 USD de crédit ≈ **~30 000 exécutions**

Avec `gpt-5` (si tu changes `MODEL_NAME`) :
- 1 exécution ≈ **0,01 USD**
- 10 USD ≈ **~1 000 exécutions**
...
> Toutes les commandes ci-dessous se lancent depuis le **dossier du projet** (celui qui contient `main.py`). Pour vérifier, lance `ls` et tu dois voir `main.py`, `pyproject.toml`, `Dockerfile`, etc.

```bash
# Verifier que tu es bien dans le bon dossier (langchain-course-project-hello-world-openai/)
ls
